...
feature:audiobookshelf  (only data change)
  ...
  AbsBookMapper                addedAtUtc = item.addedAt ?: now
  AbsCatalogReconciler         prefer server addedAt (fall back to prev only if null)

core:navigation
  CommonRoutes                 + AudiobookSearch, + AudiobookFilter constants
...
source (`observeAll()`) feeds the grid, all four sort modes, and search alike; and the
...
- `AbsLibraryItem` gains `val addedAt: Long? = null` (ABS sends epoch-ms;
  `AbsJson` has `ignoreUnknownKeys = true` so a wrong guess degrades to null).
...
```
object BookExplore {
    fun sort(books, mode): List<Book>
    fun group(books, mode): List<Section>          // Section(header: String, books: List<Book>)
    fun search(books, term): SearchResults         // (books, series, authors)
    fun filterSeries(books, name): List<Book>
    fun filterAuthor(books, name): List<Book>
...
- VM exposes `sortMode: StateFlow<BookSortMode>` (seeded from `AudiobookUiPreferences`)
  and a derived view of the library: either a flat `List<Book>` (RECENT/TITLE) or a
  `List<Section>` (AUTHOR/SERIES). `onSortSelected(mode)` updates state and persists.
- Top bar `trailing` slot → a `Row { SortMenu(); IconButton(🔍) }`.
  - `SortMenu` = an `IconButton(⇅)` that toggles a `DropdownMenu` of four
...
- When true, draw a small `Icons.Filled.DownloadDone` in a dark rounded chip at the
...
- VM holds the library list (`repository.observeAll`) and recomputes `SearchResults` as
  the term changes. UI renders labelled sections (each only if non-empty):
...
- `CommonRoutes`: add `AudiobookSearch = "audiobook-search"` and a filter route pattern
  `audiobook-filter/{type}/{value}` with a `audiobookFilter(type, value)` builder
  (`Uri.encode` the value). Mirrors `PodcastSearch`.
...
```
Room books ──observeAll()──> repository ──> VM(s)
                                   │
...
- **Empty library / no folder** → existing empty states unchanged; sort menu + search
  icon may be hidden or inert when there are no books.
- **ABS item missing `addedAt`** → mapper falls back to `now`; reconciler keeps the
  previous timestamp to avoid churn across syncs.
...
- `AbsCatalogReconciler` prefers the server `addedAt` over the previous value.
- DTO parses `addedAt` (and tolerates its absence via `ignoreUnknownKeys`).
